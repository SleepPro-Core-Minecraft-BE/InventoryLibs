...
use m1rage\invlibstest\command\TestChestCommand;
...
final class Loader extends PluginBase{
	...
	protected function onEnable() : void{
		...
		if(!$plugin instanceof LibraryLoader || !InventoryLibsHandler::isRegistered()){
			throw new RuntimeException('InventoryLibs registration failed');
		}
		$factory = $plugin->getManager()->getFactory();
		$this->getServer()->getCommandMap()->register('inventorylibs', new TestChestCommand($factory));
		foreach([
		...
		if($this->getConfig()->get('shutdown-after-test', false) === true){
