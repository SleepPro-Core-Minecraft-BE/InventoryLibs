...
final class PlayerWindowDispatcher{
	...
	/** @var array{InventoryLibs, string|null, Closure|null}|null */
	public ?array $after_finalization = null;

	public function __construct(
		readonly public PlayerSession $session,
		readonly public InventoryLibsInfo $info,
	...
	private function registerContainerOpenCallbacks() : bool{
		...
		$this->container_open_callback = function(int $window_id, Inventory $inventory) use($translator, $callbacks, $previous) : ?array{
			...
			$this->task_handler = InventoryLibsHandler::getRegistrant()->getScheduler()->scheduleRepeatingTask(new ClosureTask($this->run(...)), 1);
			return $packets;
		};
	...
	public function setResult(bool $result) : void{
		...
		if($this->n_finalization_acks > 0){
			$this->task_handler = InventoryLibsHandler::getRegistrant()->getScheduler()->scheduleDelayedTask(new ClosureTask($this->finalize(...)), $this->finalization_timeo);
		}else{
			$this->finalize();
		}
	}
	...
	public function then(InventoryLibs $menu, ?string $name, ?Closure $callback) : void{
